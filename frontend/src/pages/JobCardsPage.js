...
export default function JobCardsPage() {
  ...
  const [workTypes, setWorkTypes] = useState([]);  // Dynamic work types
  ...
  const loadData = useCallback(async () => {
    try {
      const [jobcardsRes, partiesRes, headersRes, workersRes, workTypesRes] = await Promise.all([
        ...
        API.get(`/api/inventory/headers`, { params: { page_size: 1000 } }),
  ...
  const handleCreateJobCard = async () => {
    try {
      ...
      const data = {
        ...
        // Advance Cash field
        advance_cash: formData.advance_cash ? parseFloat(formData.advance_cash) : 0,
  ...
  const handleEditJobCard = (jobcard) => {
    ...
    setFormData({
      ...
      advance_cash: jobcard.advance_cash || '',
          ...
          <div className="space-y-6 mt-4">
            ...
            {!saveAsTemplate && !editingTemplate && (
              <div className="space-y-4 border-t pt-4 mt-4">
                ...
                <div className="space-y-2">
                  <Label>Advance Cash (OMR)</Label>
                  <Input
                    data-testid="advance-cash-input"
                    type="number"
                    step="0.001"
                    min="0"
                    value={formData.advance_cash}
                    onChange={(e) => setFormData({...formData, advance_cash: e.target.value})}
                    placeholder="0.000"
                  />
                  <p className="text-xs text-gray-500">Cash received in advance before invoice generation</p>
                </div>
                ...
                {((formData.advance_in_gold_grams && formData.advance_gold_rate) || 
                  ...
                  (formData.advance_cash && parseFloat(formData.advance_cash) > 0)) && (
                  <div className="bg-purple-50 p-3 rounded-md border border-purple-200" data-testid="settlement-summary">
                    <p className="text-xs font-semibold text-purple-900 mb-2">Settlement Summary:</p>
                    {formData.advance_cash && parseFloat(formData.advance_cash) > 0 && (
                      <div className="text-xs text-purple-800" data-testid="advance-cash-summary">
                        Advance Cash: {parseFloat(formData.advance_cash).toFixed(3)} OMR
                      </div>
                    )}
                    {formData.advance_in_gold_grams && formData.advance_gold_rate && (
                      <div className="text-xs text-purple-800">
                        Advance Gold: {parseFloat(formData.advance_in_gold_grams).toFixed(3)}g × {parseFloat(formData.advance_gold_rate).toFixed(2)} = {(parseFloat(formData.advance_in_gold_grams) * parseFloat(formData.advance_gold_rate)).toFixed(3)} OMR
                      </div>
                    )}
                    {formData.exchange_in_gold_grams && formData.exchange_gold_rate && (
                      <div className="text-xs text-purple-800">
                        Exchange Gold: {parseFloat(formData.exchange_in_gold_grams).toFixed(3)}g × {parseFloat(formData.exchange_gold_rate).toFixed(2)} = {(parseFloat(formData.exchange_in_gold_grams) * parseFloat(formData.exchange_gold_rate)).toFixed(3)} OMR
                      </div>
                    )}
                    <div className="text-xs font-semibold text-purple-900 mt-2 pt-2 border-t border-purple-300" data-testid="total-deduction-summary">
                      Total Deduction: {(
                        (formData.advance_cash ? parseFloat(formData.advance_cash) : 0) +
                        (formData.advance_in_gold_grams && formData.advance_gold_rate ? parseFloat(formData.advance_in_gold_grams) * parseFloat(formData.advance_gold_rate) : 0) +
                        (formData.exchange_in_gold_grams && formData.exchange_gold_rate ? parseFloat(formData.exchange_in_gold_grams) * parseFloat(formData.exchange_gold_rate) : 0)
                      ).toFixed(3)} OMR
                    </div>
                  </div>
                )}
            ...
            {!saveAsTemplate && !editingTemplate && formData.gold_rate_at_jobcard && (
              <div className="bg-gradient-to-br from-indigo-50 via-purple-50 to-blue-50 border-2 border-indigo-300 rounded-xl p-5 mt-6 shadow-lg">
                ...
                {(() => {
                  const advanceGoldGrams = parseFloat(formData.advance_in_gold_grams) || 0;
                  const advanceGoldRate = parseFloat(formData.advance_gold_rate) || 0;
                  const exchangeGoldGrams = parseFloat(formData.exchange_in_gold_grams) || 0;
                  const exchangeGoldRate = parseFloat(formData.exchange_gold_rate) || 0;
                      ...
                      <div className="bg-gradient-to-br from-purple-100 to-purple-200 rounded-lg p-4 mb-4 border-2 border-purple-300">
                        ...
                        <div className="space-y-1 text-sm">
                          ...
                          <div className="flex justify-between font-bold text-purple-900 pt-1 border-t border-purple-300">
                            <span>Total Gold Deduction</span>
                ...
                {/* Advance Cash Deduction Section - Separate from Gold Settlement */}
                {(() => {
                  const advanceCash = parseFloat(formData.advance_cash) || 0;
                  if (advanceCash > 0) {
                    return (
                      <div className="bg-gradient-to-br from-emerald-100 to-teal-100 rounded-lg p-4 mb-4 border-2 border-emerald-300" data-testid="form-advance-cash-deduction">
                        <div className="text-xs text-emerald-800 font-medium uppercase mb-2 flex items-center gap-2">
                          <span>💵</span> Advance Cash Deduction
                        </div>
                        <div className="space-y-1 text-sm">
                          <div className="flex justify-between font-bold text-emerald-900">
                            <span>Advance Cash Received</span>
                            <span className="font-mono">-{advanceCash.toFixed(3)} OMR</span>
                          </div>
                        </div>
                      </div>
                    );
                    ...
                    <div>
                      ...
                      <div className="font-mono font-black text-4xl text-white">
                        {(() => {
                          ...
                          // Final total = items total - gold settlement deduction - advance cash
                          const finalTotal = itemsTotal - totalGoldDeduction - advanceCash;
      ...
      <Dialog open={showConvertDialog} onOpenChange={setShowConvertDialog}>
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-6">
            ...
            {/* Advance Cash Info - Read Only */}
            {convertingJobCard && parseFloat(convertingJobCard.advance_cash) > 0 && (
              <div className="p-3 bg-emerald-50 border border-emerald-200 rounded-lg text-sm" data-testid="convert-advance-cash-info">
                <span className="font-semibold text-emerald-900">💵 Advance Cash Received: </span>
                <span className="text-emerald-800 font-mono">{parseFloat(convertingJobCard.advance_cash).toFixed(3)} OMR</span>
                <p className="text-xs text-emerald-700 mt-1">This amount will be deducted from the invoice payable total</p>
              </div>
            )}
        ...
        <DialogContent className="max-w-6xl max-h-[90vh] overflow-y-auto">
          ...
          {viewJobCard && (
            <div className="space-y-6">
              ...
              {viewJobCard.gold_rate_at_jobcard && (
                <div className="bg-gradient-to-br from-indigo-50 via-purple-50 to-blue-50 border-2 border-indigo-300 rounded-xl p-5 shadow-lg">
                  ...
                  {(() => {
                    ...
                    if (totalGoldDeduction > 0) {
                      return (
                        <div className="bg-gradient-to-br from-purple-100 to-purple-200 rounded-lg p-4 mb-4 border-2 border-purple-300" data-testid="view-gold-settlement-deduction">
                          <div className="text-xs text-purple-800 font-medium uppercase mb-2 flex items-center gap-2">
                            <span>💰</span> Gold Settlement Deduction
                          </div>
                          <div className="space-y-1 text-sm">
                  ...
                  {/* Advance Cash Deduction Section - Separate from Gold Settlement */}
                  {(() => {
                    const advanceCash = parseFloat(viewJobCard.advance_cash) || 0;
                    if (advanceCash > 0) {
                      return (
                        <div className="bg-gradient-to-br from-emerald-100 to-teal-100 rounded-lg p-4 mb-4 border-2 border-emerald-300" data-testid="view-advance-cash-deduction">
                          <div className="text-xs text-emerald-800 font-medium uppercase mb-2 flex items-center gap-2">
                            <span>💵</span> Advance Cash Deduction
                          </div>
                          <div className="space-y-1 text-sm">
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <div className="text-xs text-amber-900">
                        ...
                        <ul className="list-disc list-inside space-y-0.5 text-amber-800">
                          ...
                          <li>Gold settlement (advance & exchange gold) is deducted from total</li>
                          {parseFloat(viewJobCard.advance_cash) > 0 && (
                            <li>Advance cash ({parseFloat(viewJobCard.advance_cash).toFixed(3)} OMR) is deducted from total</li>
                          )}
                          <li>Actual invoice amount may vary based on final weight and current market rates</li>
                          <li>Final costs determined at invoice conversion</li>
                        </ul>
              ...
              {/* Advance Cash Section - Separate from Gold Settlement */}
              {parseFloat(viewJobCard.advance_cash) > 0 && (
                <div className="bg-gradient-to-br from-emerald-50 to-teal-50 border-2 border-emerald-300 rounded-xl p-5 shadow-lg" data-testid="view-advance-cash-section">
                  <h3 className="font-bold text-lg text-emerald-900 flex items-center gap-2 mb-4">
                    <span className="text-2xl">💵</span>
                    Advance Cash
                  </h3>
                  <p className="text-xs text-emerald-700 mb-4">
                    Cash advance received from customer — deducted from payable total
                  </p>
                  ...
                  <div className="bg-white rounded-lg p-4 border border-emerald-200 shadow-sm">
                    <div className="flex items-center justify-between mb-2">
                      <span className="font-semibold text-emerald-900 flex items-center gap-2">
                        <span className="text-lg">💰</span> Cash Received
                      </span>
                      <Badge className="bg-emerald-100 text-emerald-800">Pre-Payment</Badge>
                    </div>
                    <div className="text-center p-3 bg-emerald-50 rounded border border-emerald-200">
                      <div className="text-xs text-emerald-600 font-medium">Amount</div>
                      <div className="font-mono font-bold text-2xl text-emerald-900" data-testid="view-advance-cash-amount">
                        {parseFloat(viewJobCard.advance_cash).toFixed(3)} OMR
                      </div>
                    </div>
                  </div>

                  {/* Info Box */}
                  <div className="mt-4 p-3 bg-emerald-100 border-l-4 border-emerald-500 rounded">
                    <div className="flex items-start gap-2">
                      <Info className="w-5 h-5 text-emerald-600 flex-shrink-0 mt-0.5" />
                      <div className="text-xs text-emerald-900">
                        <p className="font-semibold mb-1">Advance Cash Info</p>
                        <p>This cash amount will be deducted from the invoice total when this job card is converted to an invoice.</p>
                      </div>
                    </div>
                  </div>
                </div>
