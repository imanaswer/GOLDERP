...
export default function FinancePageEnhanced() {
  ...
  // Filter states - pending (UI) and applied (API)
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      const errorMsg = error.response?.data?.detail || 'Failed to delete transaction';
  ...
  const hasActiveFilters = Object.entries(pendingFilters).some(([key, value]) => {
          ...
          <Card className="bg-gradient-to-br from-green-50 to-green-100 border-green-200">
            <CardContent className="p-6">
              ...
              <div className={`text-3xl font-bold ${summary.cash_summary.net >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                {summary.cash_summary.net >= 0 ? '+' : ''}{formatCurrency(summary.cash_summary?.net)}
              </div>
          ...
          <Card className="bg-gradient-to-br from-purple-50 to-purple-100 border-purple-200">
            <CardContent className="p-6">
              ...
              <div className={`text-3xl font-bold ${summary.bank_summary.net >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                {summary.bank_summary.net >= 0 ? '+' : ''}{formatCurrency(summary.bank_summary?.net)}
              </div>
              <div className="text-xs text-purple-600 mt-1">OMR</div>
              <div className="mt-3 pt-3 border-t border-purple-300">
                <div className="flex justify-between text-sm mb-1">
                  <span className="text-green-600">In</span>
                  <span className="font-mono font-semibold">{formatCurrency(summary.bank_summary?.credit || 0)}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-red-600">Out</span>
                  <span className="font-mono font-semibold">{formatCurrency(summary.bank_summary?.debit || 0)}</span>
                </div>
              </div>
            </CardContent>
          </Card>
            ...
            <div className="px-6 pb-4 border-b bg-muted/30">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  <Select value={pendingFilters.account_type} onValueChange={(val) => setPendingFilters({...pendingFilters, account_type: val})}>
                    <SelectTrigger className="mt-1">
              ...
              <div className="mt-3 flex justify-end gap-2">
                ...
                <Button size="sm" onClick={applyFinanceFilters} data-testid="apply-filters-btn">
                  Apply Filters
                </Button>
              </div>
            </div>
