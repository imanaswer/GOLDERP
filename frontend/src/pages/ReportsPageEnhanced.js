...
export default function ReportsPageEnhanced() {
  ...
  // Define all useCallback functions BEFORE useEffect hooks
  ...
  const loadCategories = useCallback(async () => {
    try {
      const response = await API.get(`/api/inventory/headers`, { params: { page_size: 1000 } });
  ...
  }, [selectedPartyId, partyType, startDate, endDate]);
  ...
  }, [startDate, endDate, movementType, category, sortBy]);

  const loadPartiesReport = useCallback(async () => {
    try {
      setLoading(true);
      const params = {};
      if (partyType && partyType !== 'all') params.party_type = partyType;
  ...
  const loadInvoicesReport = useCallback(async () => {
    try {
      setLoading(true);
      const params = {};
      if (startDate) params.start_date = startDate;
  ...
  }, [startDate, endDate, invoiceType, paymentStatus, selectedPartyId, sortBy]);
  ...
  }, [startDate, endDate, transactionType, selectedPartyId, sortBy]);

  const loadSalesHistoryReport = useCallback(async () => {
    try {
      setLoading(true);
      const params = {};
  ...
  }, [startDate, endDate, selectedPartyId, searchQuery]);

  const loadPurchaseHistoryReport = useCallback(async () => {
    try {
      setLoading(true);
      const params = {};
      if (startDate) params.date_from = startDate;
  ...
  }, [startDate, endDate, selectedPartyId, purchaseSearchQuery]);
  ...
  // Now useEffect hooks can safely use the callbacks defined above
  useEffect(() => {
    loadFinancialSummary();
    loadCategories();
    loadAccounts();
    loadParties();
  }, [loadFinancialSummary, loadCategories, loadAccounts, loadParties]);
  ...
  }, [activeTab, loadFinancialSummary, loadOutstandingReport, loadInventoryReport, loadPartiesReport, loadInvoicesReport, loadTransactionsReport, loadSalesHistoryReport, loadPurchaseHistoryReport, loadReturnsReport, loadReturnsSummary]);
