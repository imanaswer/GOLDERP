...
async def execute_in_transaction(operations_func, *args, **kwargs):
    ...
    This ensures ALL database operations succeed together or fail together,
    preventing partial writes that could corrupt financial data.
    ...
    Raises:
        HTTPException: If any operation within the transaction fails
    
    Usage:
        async def my_operations(session, arg1, arg2):
            await db.collection.insert_one(doc, session=session)
            await db.collection.update_one(filter, update, session=session)
            return result
        
        result = await execute_in_transaction(my_operations, arg1, arg2)
    """
    async with await client.start_session() as session:
        async with session.start_transaction():
            try:
                ...
                # Re-raise HTTP exceptions (they contain user-friendly error messages)
                raise
