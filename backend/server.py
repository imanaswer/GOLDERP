...
# ============================================================================
# GLOBAL FIX: Register Decimal128/ObjectId in FastAPI's encoder registry
# This ensures jsonable_encoder handles these types BEFORE JSON serialization
# ============================================================================
ENCODERS_BY_TYPE[Decimal128] = lambda v: float(v.to_decimal())
ENCODERS_BY_TYPE[ObjectId] = str

# ============================================================================
# CUSTOM JSON ENCODER FOR AUTOMATIC DECIMAL128 CONVERSION
# ============================================================================
class CustomJSONEncoder(json.JSONEncoder):
    """
    Custom JSON encoder that automatically converts:
    - Decimal128 → float (for monetary and weight values)
    - datetime → ISO format string
    - ObjectId → string
    - Decimal → float
    
    This ensures all MongoDB types are JSON-serializable without manual conversion.
    """
...
def auto_convert_response(data):
    """
    Recursively convert Decimal128, ObjectId, and datetime objects in any data structure.
    This is a deep conversion that handles nested dicts, lists, and objects.
    """
    if isinstance(data, dict):
        return {k: auto_convert_response(v) for k, v in data.items()}
    elif isinstance(data, list):
        return [auto_convert_response(item) for item in data]
...
# ============================================================================
# CUSTOM JSON RESPONSE CLASS - Global safety net for Decimal128 serialization
# ============================================================================
class CustomJSONResponse(JSONResponse):
    """JSONResponse that auto-converts Decimal128/ObjectId before serialization."""
    def render(self, content: Any) -> bytes:
        converted = auto_convert_response(content)
        return json.dumps(
            converted,
            ensure_ascii=False,
            allow_nan=False,
...
# ============================================================================
# All financial operations MUST be wrapped in transactions for ACID guarantees
# This prevents partial writes and maintains data integrity

def _is_transaction_error(e: Exception) -> bool:
    """Check if an exception is related to MongoDB transaction/replica set limitations."""
    error_msg = str(e).lower()
    return "transaction" in error_msg or "replica" in error_msg or "session" in error_msg

async def execute_in_transaction(operations_func, *args, **kwargs):
    """
    Execute a function within a MongoDB transaction for atomicity.
    
    Falls back to non-transactional execution when MongoDB is not configured
    as a replica set (standalone mode). Logs a warning when fallback is used.
    
    Args:
        operations_func: Async function to execute within transaction.
                         Must accept `session` as first arg (session=None in fallback).
        *args, **kwargs: Arguments to pass to operations_func
    
    Returns:
        Result from operations_func
    """
    try:
        async with await client.start_session() as session:
            async with session.start_transaction():
                try:
                    result = await operations_func(session, *args, **kwargs)
                    return result
                except HTTPException:
                    raise
                except Exception as e:
                    if _is_transaction_error(e):
                        raise  # Let it propagate to the outer handler for fallback
        ...
        if _is_transaction_error(e):
            logging.warning(
                "MongoDB transactions not supported (not a replica set). "
                "Falling back to non-transactional execution."
            )
            try:
                return await operations_func(None, *args, **kwargs)
            except HTTPException:
                raise
            except Exception as inner_e:
                logging.error(f"Non-transactional operation failed: {str(inner_e)}")
                raise HTTPException(
                    status_code=500,
                    detail=f"Operation failed: {str(inner_e)}"
                )
        else:
            logging.error(f"Transaction setup failed: {str(e)}")
            raise HTTPException(
                status_code=500,
                detail=f"Operation failed: {str(e)}"
            )
...
def calculate_balance_delta(account_type: str, transaction_type: str, amount: float) -> float:
    ...
    SIMPLIFIED CASH-FLOW SYSTEM (matches UI labels):
    - CREDIT = Money IN → increases balance (+)
    - DEBIT = Money OUT → decreases balance (-)
    
    This applies to ALL account types consistently.
    
    Examples:
    - Any Account + Credit $100 = +$100 balance (Money IN)
    - Any Account + Debit $100 = -$100 balance (Money OUT)
    
    Note: This is a simplified system that matches the UI terminology where
    users see "Credit (Money IN)" and "Debit (Money OUT)" in the transaction form.
    """
    # Simplified system: Credit = Money IN (+), Debit = Money OUT (-)
    # This is consistent across ALL account types (Asset, Income, Expense, etc.)
...
def safe_float(value):
    """Safely convert a value to float, handling Decimal128, int, float, and None"""
    if value is None:
        return 0.0
...
class JobCard(BaseModel):
    ...
    advance_cash: Optional[float] = None  # Advance cash received from customer (OMR, ≥ 0)
...
class Return(BaseModel):
    ...
    party_id: Optional[str] = None  # Customer (for sale_return) or Vendor (for purchase_return)
    party_name: Optional[str] = None  # Party name for display
...
async def create_audit_log(user_id: str, user_name: str, module: str, record_id: str, action: str, changes: Optional[Dict] = None, session=None):
    log = AuditLog(
        user_id=user_id,
        user_name=user_name,
        module=module,
    ...
    await db.audit_logs.insert_one(log.model_dump(), session=session)
...
async def get_current_user(
    ...
    try:
        ...
        return User(**decimal_to_float(user_doc))
...
async def login(request: Request, credentials: UserLogin, response: Response):
    ...
    user = User(**decimal_to_float(user_doc))
...
@api_router.put("/inventory/headers/{header_id}", response_model=InventoryHeader)
async def update_inventory_header(
   ...
   # Prepare update data - only allow updating name and is_active
    ...
    return InventoryHeader(**decimal_to_float(updated_header))
...
async def delete_inventory_header(header_id: str, current_user: User = Depends(require_permission('inventory.adjust'))):
    ...
    current_qty = safe_float(existing_header.get('current_qty', 0))
    current_weight = safe_float(existing_header.get('current_weight', 0))
    
    if current_qty > 0 or current_weight > 0:
        raise HTTPException(
            status_code=400, 
            detail=f"Cannot delete inventory header with existing stock. Current: {current_qty} qty, {current_weight}g"
        )
...
async def create_stock_movement(movement_data: dict, current_user: User = Depends(require_permission('inventory.adjust'))):
    ...
    # Convert Decimal128 to float before arithmetic to avoid serialization issues
    current_qty_float = safe_float(header.get('current_qty', 0))
    current_weight_float = safe_float(header.get('current_weight', 0))
    
    new_qty = current_qty_float + qty_delta
    new_weight = current_weight_float + weight_delta
...
async def delete_stock_movement(movement_id: str, current_user: User = Depends(require_permission('inventory.adjust'))):
    ...
    current_qty_float = safe_float(header.get('current_qty', 0))
    current_weight_float = safe_float(header.get('current_weight', 0))
    movement_qty_float = safe_float(movement['qty_delta'])
    movement_weight_float = safe_float(movement['weight_delta'])
    
    new_qty = current_qty_float - movement_qty_float
    new_weight = current_weight_float - movement_weight_float
    
    # Validate reversal won't make stock negative
    if new_qty < 0 or new_weight < 0:
        raise HTTPException(
            status_code=400,
            detail=f"Cannot delete movement: would result in negative stock. Current: {current_qty_float} qty, {current_weight_float}g. Movement: {movement_qty_float} qty, {movement_weight_float}g"
        )
    ...
    await create_audit_log(
        ...
        changes={
            ...
            "qty_delta": safe_float(movement.get('qty_delta')),
            "weight_delta": safe_float(movement.get('weight_delta')),
    ...
    return {
        ...
        "id": movement_id,
        "reversed_qty": safe_float(movement['qty_delta']),
        "reversed_weight": safe_float(movement['weight_delta'])
    }
...
async def get_stock_totals(current_user: User = Depends(require_permission('inventory.view'))):
    ...
    # Convert Decimal128 to float before returning
    return [
        decimal_to_float({
            "header_id": h['id'], 
            "header_name": h['name'], 
...
async def get_inventory(
    ...
    try:
        ...
        if min_qty is not None:
            ...
            # Convert Decimal128 to float for filtering and comparisons
            headers = [decimal_to_float(h) for h in headers]
...
async def get_outstanding_summary(current_user: User = Depends(require_permission('parties.view'))):
    ...
    for inv in invoices:
        ...
        if cid:
            ...
            # Add balance_due (already converted to float)
...
async def get_party(party_id: str, current_user: User = Depends(require_permission('parties.view'))):
    ...
    return Party(**decimal_to_float(party))
...
async def update_party(party_id: str, party_data: dict, current_user: User = Depends(require_permission('parties.update'))):
    ...
    return Party(**decimal_to_float(updated))
...
async def get_party_impact(party_id: str, current_user: User = Depends(require_permission('parties.view'))):
    ...
    # Convert Decimal128 to float for calculations
    gold_entries = [decimal_to_float(entry) for entry in gold_entries]
    gold_balance = 0
...
async def get_worker(worker_id: str, current_user: User = Depends(get_current_user)):
    ...
    return Worker(**decimal_to_float(worker))
...
async def get_work_type(work_type_id: str, current_user: User = Depends(get_current_user)):
    ...
    return WorkType(**decimal_to_float(work_type))
...
async def get_gold_deposits(
    ...
    return [decimal_to_float(e) for e in entries]
...
async def get_party_gold_summary(party_id: str, current_user: User = Depends(require_permission('parties.view'))):
    ...
    # Convert Decimal128 to float for calculations
    entries = [decimal_to_float(entry) for entry in entries]
...
async def create_purchase(request: Request, purchase_data: dict, current_user: User = Depends(require_permission('purchases.create'))):
    ...
    if items_list and len(items_list) > 0:
        ...
        for idx, item in enumerate(items_list):
            ...
            # ✅ CORRECT PURCHASE AMOUNT FORMULA: Amount = ((Weight × (Purity / 916)) ÷ Conversion_Factor) × Rate
            # Step-by-step calculation:
            # Step 1: Normalize purity to 22K (916)
            purity_ratio = Decimal(str(purity)) / Decimal("916")
            adjusted_weight = Decimal(str(weight)) * purity_ratio
            # Step 2: Apply conversion factor
            converted_weight = adjusted_weight / Decimal(str(conversion_factor))
            # Step 3: Calculate amount
            item_amount = converted_weight * Decimal(str(rate))
            item_amount = round(float(item_amount), 3)  # 3 decimal precision (Oman)
        ...
        # Step-by-step calculation:
        # Step 1: Normalize purity to 22K (916)
        purity_ratio = Decimal(str(entered_purity)) / Decimal("916")
        adjusted_weight = Decimal(str(weight_grams)) * purity_ratio
        # Step 2: Apply conversion factor
        converted_weight = adjusted_weight / Decimal(str(conversion_factor))
        # Step 3: Calculate amount
        calculated_total = converted_weight * Decimal(str(rate_per_gram))
        calculated_total = round(float(calculated_total), 3)  # 3 decimal precision (Oman)
    ...
    # Round both to 3 decimal places before comparison to avoid floating-point precision mismatch
    paid_rounded = round(paid_amount, 3)
    total_rounded = round(float(purchase_data["amount_total"]), 3)
    
    if paid_rounded > total_rounded:
        raise HTTPException(status_code=400, detail=f"Paid amount ({paid_rounded}) cannot exceed total amount ({total_rounded})")
    
    purchase_data["paid_amount_money"] = paid_rounded
    balance = round(total_rounded - paid_rounded, 3)
    purchase_data["balance_due_money"] = max(balance, 0.0)  # Prevent -0.00
    ...
    # ========== TRANSACTION WRAPPER: ALL DATABASE OPERATIONS ATOMIC ==========
    # ALL operations below MUST succeed together or fail together
    # This prevents partial purchase creation that could corrupt financial data
    
    async def create_purchase_with_entries(session):
        """Execute all purchase creation operations atomically within a transaction"""
        # Convert to Decimal128 for precise storage
        purchase_data_dec = convert_purchase_to_decimal(purchase.model_dump())
        await db.purchases.insert_one(purchase_data_dec, session=session)
        
        # ========== AUTO-FINALIZATION: CREATE ALL ACCOUNTING ENTRIES ==========
        
        # === OPERATION 1: Create Stock IN movements (for each item or single legacy item) ===
        if items_list and len(items_list) > 0:
            # Multiple items: Create separate stock movements for each
            for item in purchase_data["items"]:
                purity = purchase_data["valuation_purity_fixed"]  # Always 916
                header_name = f"Gold {purity // 41.6:.0f}K"  # 916 = 22K
                
                header = await db.inventory_headers.find_one({"name": header_name, "is_deleted": False}, session=session)
                if not header:
                    ...
                    await db.inventory_headers.insert_one(header.model_dump(), session=session)
                
                # Create Stock IN movement for this item
                movement = StockMovement(
                    date=purchase.date,
                    ...
                    created_by=current_user.username,
                    notes=f"Calculation: Weight={item['weight_grams']}g × ({item['entered_purity']}/916) ÷ {conversion_factor} × Rate={item['rate_per_gram_22k']} OMR/g = {item['calculated_amount']} OMR | Entered purity: {item['entered_purity']}, Valuation purity: {purity}"
                )
                movement_dict = convert_stock_movement_to_decimal(movement.model_dump())
                await db.stock_movements.insert_one(movement_dict, session=session)
                ...
                await db.inventory_headers.update_one(
                    ...
                    {"$inc": {
                        "current_qty": 1,
                        "current_weight": item["weight_grams"]
                    }},
                    session=session
                )
        else:
            ...
            movement = StockMovement(
                ...
                header_id=header.get("id") if isinstance(header, dict) else header.id,
                header_name=header_name,
                description=f"Purchase from {vendor_name}: {purchase_data.get('description', '')}",
                qty_delta=1,
                ...
                reference_id=purchase_id,
                created_by=current_user.username,
                notes=f"Calculation: (Weight={purchase.weight_grams}g × ({purchase.entered_purity}/916) ÷ Factor={conversion_factor}) × Rate={purchase_data.get('rate_per_gram')} OMR/g = {purchase_data['amount_total']} OMR | Entered purity: {purchase.entered_purity}, Valuation purity: {purity}"
            )
            movement_dict = convert_stock_movement_to_decimal(movement.model_dump())
            await db.stock_movements.insert_one(movement_dict, session=session)
        ...
        # === OPERATION 2: Create DEBIT transaction if paid_amount_money > 0 (Double-Entry) ===
        if purchase_data["paid_amount_money"] > 0:
            current_year = datetime.now(timezone.utc).year
            existing_txns = await db.transactions.count_documents({"transaction_number": {"$regex": f"^TXN-{current_year}-"}}, session=session)
            payment_txn_number = f"TXN-{current_year}-{existing_txns + 1:04d}"
            credit_txn_number = f"TXN-{current_year}-{existing_txns + 2:04d}"
            
            account = await db.accounts.find_one({"id": purchase_data["account_id"], "is_deleted": False}, session=session)
            
            # Transaction 1: DEBIT Cash/Bank (Money OUT)
            payment_transaction = Transaction(
                ...
                category="Purchase Payment - Cash/Bank (Debit)",
                reference_type="purchase",
                reference_id=purchase_id,
            ...
            # Convert to Decimal128 for precise storage
            await db.transactions.insert_one(convert_transaction_to_decimal(payment_transaction.model_dump()), session=session)
            
            # Update account balance (DEBIT = Money OUT, balance decreases)
            ...
            # Transaction 2: CREDIT Purchases account (Double-Entry counterpart)
            purchases_acct = await db.accounts.find_one({"name": "Purchases", "is_deleted": False}, session=session)
            if not purchases_acct:
                purchases_acct = Account(
                    name="Purchases",
                    account_type="expense",
                    balance=0,
                    created_by=current_user.username
                )
                await db.accounts.insert_one(purchases_acct.model_dump(), session=session)
                purchases_acct = purchases_acct.model_dump()
            ...
            credit_transaction = Transaction(
                ...
                mode=purchase_data.get("payment_mode", "Cash"),
                account_id=purchases_acct.get("id") if isinstance(purchases_acct, dict) else purchases_acct.id,
                account_name="Purchases",
                party_id=vendor_party_id,
                party_name=vendor_name,
                amount=round(purchase_data["paid_amount_money"], 3),
                category="Purchase Payment - Purchases (Credit)",
                reference_type="purchase",
                reference_id=purchase_id,
                notes=f"Purchase expense for {vendor_name} ({purchase_data['weight_grams']}g total)",
                created_by=current_user.username
            )
            await db.transactions.insert_one(convert_transaction_to_decimal(credit_transaction.model_dump()), session=session)
            
            # Update Purchases account balance
            await db.accounts.update_one(
                {"id": purchases_acct.get("id") if isinstance(purchases_acct, dict) else purchases_acct.id},
                {"$inc": {"current_balance": round(purchase_data["paid_amount_money"], 3)}},
                session=session
            )
        ...
        if not is_walk_in and vendor_party_id:
            ...
            if advance_gold and advance_gold > 0:
                ...
                await db.gold_ledger.insert_one(convert_gold_ledger_to_decimal(advance_entry.model_dump()), session=session)
        ...
        if not is_walk_in and vendor_party_id:
            ...
            if exchange_gold and exchange_gold > 0:
                ...
                await db.gold_ledger.insert_one(convert_gold_ledger_to_decimal(exchange_entry.model_dump()), session=session)
        ...
        if balance_due > 0 and not is_walk_in and vendor_party_id:
            ...
            purchases_account = await db.accounts.find_one({"name": "Purchases", "is_deleted": False}, session=session)
            if not purchases_account:
                ...
                await db.accounts.insert_one(purchases_account.model_dump(), session=session)
            ...
            # Convert to Decimal128 for precise storage
            await db.transactions.insert_one(convert_transaction_to_decimal(payable_transaction.model_dump()), session=session)
    ...
    return await execute_in_transaction(create_purchase_with_entries)
...
async def add_payment_to_purchase(
    ...
    payment_amount = round(float(payment_amount), 3)
    ...
    new_paid_amount = round(purchase.paid_amount_money + payment_amount, 3)
    new_balance_due = round(purchase.amount_total - new_paid_amount, 3)
    ...
    should_lock = (new_balance_due <= 0.001)
    ...
    # DOUBLE-ENTRY BOOKKEEPING for Purchase Payment:
    # Transaction 1: DEBIT Cash/Bank (Money OUT - balance decreases)
    payment_transaction = Transaction(
        transaction_number=payment_txn_number,
    ...
    # Transaction 2: CREDIT Purchases account (Double-Entry counterpart)
    purchases_acct = await db.accounts.find_one({"name": "Purchases", "is_deleted": False})
    if not purchases_acct:
        purchases_acct = {
            "id": str(uuid.uuid4()),
            "name": "Purchases",
        ...
        await db.accounts.insert_one(purchases_acct)
    ...
    credit_transaction = Transaction(
        ...
        reference_type="purchase",
        reference_id=purchase_id,
        notes=f"Purchase expense payment: {purchase.description}",
        created_by=current_user.username
    )
    await db.transactions.insert_one(convert_transaction_to_decimal(credit_transaction.model_dump()))
...
async def update_purchase(
    ...
    if has_multiple_items:
        ...
        if "items" in updates:
            ...
            calculated_total = safe_float(amount_total_value)
        ...
        # Get current values or updated values with safe conversion
        weight_grams_raw = updates.get("weight_grams", existing.get("weight_grams", 0))
        rate_per_gram_raw = updates.get("rate_per_gram", existing.get("rate_per_gram", 0))
        
        # Ensure proper type conversion from database (handles Decimal128)
        weight_grams = safe_float(weight_grams_raw)
        rate_per_gram = safe_float(rate_per_gram_raw)
        ...
        if "rate_per_gram" in updates:
            ...
            updates["rate_per_gram"] = round(rate_per_gram, 3)
        
        # RECALCULATE total_amount on backend (SINGLE SOURCE OF TRUTH)
        # Never trust client-sent total_amount
        # Extract purity for calculation (from updates or existing record)
        entered_purity_raw = updates.get("entered_purity", existing.get("entered_purity", 916))
        entered_purity = int(safe_float(entered_purity_raw)) or 916
            
        # Extract conversion factor (from updates or existing record, fallback to 0.920)
        conversion_factor_raw = updates.get("conversion_factor", existing.get("conversion_factor", 0.920))
        conversion_factor = safe_float(conversion_factor_raw)
        
        # ⚠️ CRITICAL FIX: Prevent division by zero
        if conversion_factor == 0 or conversion_factor is None:
            conversion_factor = 0.920
        
        # ✅ SAFE CALCULATION BLOCK WITH PROPER ERROR HANDLING
        # Formula: Amount = ((Weight × (Purity / 916)) ÷ Conversion_Factor) × Rate
        try:
            # Ensure all values are properly converted to float before calculation
            weight = safe_float(weight_grams)
            purity = safe_float(entered_purity)
            rate = safe_float(rate_per_gram)
            conv_factor = safe_float(conversion_factor)
            
            # Validate conversion factor is not zero (defensive check)
            if conv_factor == 0:
                conv_factor = 0.920
            
            # Step 1: Normalize purity to 22K (916)
            purity_adjustment = purity / 916.0
            # Step 2: Apply purity adjustment to weight
            adjusted_weight = weight * purity_adjustment
            # Step 3: Apply conversion factor
            converted_weight = adjusted_weight / conv_factor
            # Step 4: Calculate total amount
            calculated_total = converted_weight * rate
            
            # Round to 3 decimal precision (Oman standard)
            final_calculated_total = round(calculated_total, 3)
            updates["amount_total"] = final_calculated_total
            
            print(f"DEBUG calculation: weight={weight}, purity={purity}, rate={rate}, conv_factor={conv_factor}, total={final_calculated_total}")
            
        except Exception as e:
            # Detailed error logging to identify the exact issue
            print(f"CRITICAL CALCULATION ERROR in update_purchase: {str(e)}")
            print(f"DEBUG values: weight_grams={weight_grams}, entered_purity={entered_purity}, rate_per_gram={rate_per_gram}, conversion_factor={conversion_factor}")
            raise HTTPException(
                status_code=500, 
                detail=f"Calculation failed: {str(e)}. Please check the input values."
            )
    
    # Get final calculated total for validation
    # Ensure we always have a calculated_total value 
    if "amount_total" in updates:
        final_calculated_total = updates["amount_total"]
    else:
        # If amount_total not in updates, get from existing record
        final_calculated_total = existing.get("amount_total", 0)
    
    # Validate paid amount
    paid_amount = safe_float(updates.get("paid_amount_money", existing.get("paid_amount_money", 0)))
    if "paid_amount_money" in updates:
        ...
        # Round both to 3 decimal places before comparison to avoid floating-point precision mismatch
        paid_rounded = round(paid_amount, 3)
        total_rounded = round(safe_float(final_calculated_total), 3)
        
        if paid_rounded > total_rounded:
            raise HTTPException(status_code=400, detail=f"Paid amount ({paid_rounded}) cannot exceed total amount ({total_rounded})")
        
        updates["paid_amount_money"] = paid_rounded
    
    # Auto-calculate balance_due_money
    balance = round(round(safe_float(final_calculated_total), 3) - round(safe_float(paid_amount), 3), 3)
    updates["balance_due_money"] = max(balance, 0.0)  # Prevent -0.00
    ...
    # Create audit log (convert any Decimal128 values for safe serialization)
    await create_audit_log(
        user_id=current_user.id,
        user_name=current_user.username,
        module="purchases",
        record_id=purchase_id,
        action="update",
        changes=decimal_to_float(updates)
    )
...
async def get_purchase_impact(purchase_id: str, current_user: User = Depends(require_permission('purchases.view'))):
    ...
    # Convert Decimal128 to float for calculations and round() calls
...
async def get_jobcard(jobcard_id: str, current_user: User = Depends(require_permission('jobcards.view'))):
    ...
    return decimal_to_float(jobcard)
...
async def create_jobcard(jobcard_data: dict, current_user: User = Depends(require_permission('jobcards.create'))):
    ...
    # Validate advance_cash field
    advance_cash = jobcard_data.get("advance_cash")
    if advance_cash is not None:
        try:
            advance_cash = round(float(advance_cash), 3)
        except (ValueError, TypeError):
            raise HTTPException(status_code=400, detail="advance_cash must be a valid number")
        if advance_cash < 0:
            raise HTTPException(status_code=400, detail="advance_cash must be >= 0")
        jobcard_data["advance_cash"] = advance_cash
    else:
        jobcard_data["advance_cash"] = 0.0
    ...
    # Ledger: Record advance cash credit to customer if advance_cash > 0 and saved customer
    if jobcard.advance_cash and jobcard.advance_cash > 0 and jobcard.customer_type == "saved" and jobcard.customer_id:
        txn_count = await db.transactions.count_documents({})
        txn_number = f"TXN-{txn_count + 1:06d}"
        # Find Cash account for the transaction
        cash_account = await db.accounts.find_one({"name": "Cash", "is_deleted": False}, {"_id": 0})
        account_id = cash_account["id"] if cash_account else None
        account_name = cash_account["name"] if cash_account else "Cash"
        
        txn = Transaction(
            transaction_number=txn_number,
            transaction_type="credit",
            mode="Cash",
            account_id=account_id or "cash",
            account_name=account_name,
            party_id=jobcard.customer_id,
            party_name=jobcard.customer_name,
            amount=jobcard.advance_cash,
            category="Advance Cash",
            notes=f"Advance cash received for Job Card {jobcard.job_card_number}",
            reference_type="jobcard",
            reference_id=jobcard.id,
            created_by=current_user.id
        )
        txn_dict = convert_transaction_to_decimal(txn.model_dump())
        await db.transactions.insert_one(txn_dict)
        
        # Update Cash account balance if found
        if cash_account:
            new_balance = safe_float(cash_account.get("current_balance", 0)) + jobcard.advance_cash
            await db.accounts.update_one(
                {"id": cash_account["id"]},
                {"$set": {"current_balance": _safe_decimal128(new_balance, '0.001')}}
            )
...
@api_router.put("/jobcards/{jobcard_id}")
...
async def convert_jobcard_to_invoice(jobcard_id: str, invoice_data: dict, current_user: User = Depends(require_permission('jobcards.create'))):
    ...
    # Adjust balance_due by deducting gold settlement values and advance cash
    advance_cash_amount = safe_float(jobcard.get('advance_cash', 0))
    balance_due = round(grand_total - advance_gold_value - exchange_gold_value - advance_cash_amount, 3)
    
    # paid_amount = advance_cash (auto-populated from job card)
    paid_amount = round(advance_cash_amount, 3)
    ...
    invoice_dict = {
        ...
        "paid_amount": paid_amount,  # Auto-populated from advance cash
        "balance_due": balance_due,  # Adjusted for gold settlement + advance cash
        ...
        "customer_oman_id": jobcard.get("customer_oman_id"),  # Carry forward customer_oman_id from job card
        "created_by": current_user.id
    }
    
    # Add notes about gold settlement and advance cash if applicable
    notes_parts = []
    ...
    if advance_cash_amount > 0:
        notes_parts.append(f"Advance Cash: {advance_cash_amount:.3f} OMR applied")
    if notes_parts:
...
@api_router.put("/jobcard-templates/{template_id}")
...
async def get_invoice(invoice_id: str, current_user: User = Depends(require_permission('invoices.view'))):
    ...
    if not invoice:
        raise HTTPException(status_code=404, detail="Invoice not found")
    return Invoice(**decimal_to_float(invoice))
...
async def finalize_invoice(invoice_id: str, current_user: User = Depends(require_permission('invoices.finalize'))):
    ...
    if is_sale_invoice:
        for item in invoice.items:
            # CRITICAL FIX: ALWAYS create Stock OUT movement for items with weight > 0
            # This ensures complete audit trail and accurate inventory reports
            if item.weight > 0:
                # Determine header details for the movement
                ...
                if item.category:
                    ...
                    if header:
                        ...
                        # Convert Decimal128 to float before arithmetic to avoid serialization issues
                        current_qty = safe_float(header.get('current_qty', 0))
                        current_weight = safe_float(header.get('current_weight', 0))
...
async def add_payment_to_invoice(
    ...
    if payment_mode == "GOLD_EXCHANGE":
        ...
        if gold_summary_list:
            ...
            # Auto-convert Decimal128 to float for calculations
            if isinstance(gold_in, Decimal128):
                gold_in = float(gold_in.to_decimal())
            if isinstance(gold_out, Decimal128):
                gold_out = float(gold_out.to_decimal())
        ...
        # Insert credit transaction with Decimal128 conversion
        await db.transactions.insert_one(convert_transaction_to_decimal(credit_transaction.model_dump()))
    ...
    # Standard payment modes (Cash, Bank Transfer, Card, UPI/Online, Cheque)
        ...
        return {
            ...
            "transaction_number": debit_txn_number,  # Primary transaction reference for frontend
...
async def get_invoice_full_details(invoice_id: str, current_user: User = Depends(require_permission('invoices.view'))):
    ...
    # Convert Decimal128 to float for calculations and JSON serialization
    invoice = decimal_to_float(invoice)
    payments = [decimal_to_float(p) for p in payments]
...
async def get_shop_settings(current_user: User = Depends(get_current_user)):
    ...
    return ShopSettings(**decimal_to_float(settings))
...
async def get_account(account_id: str, current_user: User = Depends(require_permission('finance.view'))):
    ...
    return decimal_to_float(account)
...
async def update_account(account_id: str, update_data: dict, current_user: User = Depends(require_permission('finance.create'))):
    ...
    await create_audit_log(current_user.id, current_user.full_name, "account", account_id, "update", decimal_to_float(update_data))
...
async def create_transaction(transaction_data: dict, current_user: User = Depends(require_permission('finance.create'))):
    ...
    Simplified cash-flow system:
    - CREDIT = Money IN → increases account balance
    - DEBIT = Money OUT → decreases account balance
    
    This applies consistently to all account types (Asset, Income, Expense, etc.)
    """
    year = datetime.now(timezone.utc).year
    count = await db.transactions.count_documents({"transaction_number": {"$regex": f"^TXN-{year}"}})
    transaction_number = f"TXN-{year}-{str(count + 1).zfill(4)}"
...
async def get_transactions_summary(
    ...
    try:
        ...
        # Cash vs Bank breakdown - using transaction mode field
        cash_credit = 0.0
        cash_debit = 0.0
        bank_credit = 0.0
        bank_debit = 0.0
        ...
        for txn in transactions:
            try:
                ...
                txn_mode = txn.get('mode', 'cash').lower()  # Get transaction mode (cash/bank/card)
                ...
                # Calculate cash vs bank flows based on transaction MODE
                # Mode directly indicates how the payment was made
                if txn_mode == 'cash':
                    if txn_type == 'credit':
                        cash_credit += amount
                    else:
                        cash_debit += amount
                elif txn_mode in ['bank', 'card']:  # Both bank and card are bank-related
                    if txn_type == 'credit':
                        bank_credit += amount
                    else:
                        bank_debit += amount
        ...
        # Get accounts to determine account types for breakdown
        try:
            accounts = await db.accounts.find({"is_deleted": False}, {"_id": 0}).to_list(1000)
        ...
        # ACCOUNTING INVARIANT (as per standard rules):
        # Credit = +amount (Money IN)
        # Debit = -amount (Money OUT)
        # Net Flow = Credits - Debits
        
        # Calculate net flows using standard accounting: Credit - Debit
        cash_net = cash_credit - cash_debit  # Credits (IN) - Debits (OUT)
        bank_net = bank_credit - bank_debit  # Credits (IN) - Debits (OUT)
        
        # Calculate total IN/OUT and NET based on ALL transactions
        # Use total_credit and total_debit which include all transaction types and modes
        total_in = total_credit   # Money IN (all credit transactions)
        total_out = total_debit   # Money OUT (all debit transactions)
        net_flow = total_credit - total_debit  # Net = ALL Credits - ALL Debits
        
        # Note: Cash Net + Bank Net may not equal Total Net if there are transactions
        # with other payment modes (e.g., 'adjustment', 'other')
        
        return {
            "total_credit": round(total_credit, 3),
            "total_debit": round(total_debit, 3),
            "net_flow": round(net_flow, 3),
            "total_in": round(total_in, 3),   # Money IN (credit transactions)
            "total_out": round(total_out, 3),  # Money OUT (debit transactions)
            "transaction_count": len(transactions),
            "cash_summary": {
                "credit": round(cash_credit, 3),  # Money IN via cash (credit = +amount)
                "debit": round(cash_debit, 3),    # Money OUT via cash (debit = -amount)
                "net": round(cash_credit - cash_debit, 3)  # Credits (IN) - Debits (OUT)
            },
            "bank_summary": {
                "credit": round(bank_credit, 3),  # Money IN via bank (credit = +amount)
                "debit": round(bank_debit, 3),    # Money OUT via bank (debit = -amount)
                "net": round(bank_credit - bank_debit, 3)  # Credits (IN) - Debits (OUT)
            },
...
async def get_daily_closings(current_user: User = Depends(require_permission('finance.view'))):
    try:
        ...
        # Convert Decimal128 to float before Pydantic validation
        return [auto_convert_response(closing) for closing in closings] if closings else []
    except Exception as e:
...
async def create_daily_closing(closing_data: dict, current_user: User = Depends(require_permission('finance.create'))):
    ...
    try:
        ...
        if needs_calculation:
            ...
            # Calculate total credit and debit (handle Decimal128 conversion)
            total_credit = round(sum(
                float(t['amount'].to_decimal()) if isinstance(t['amount'], Decimal128) else float(t['amount'])
                for t in transactions if t.get('transaction_type') == 'credit'
            ), 3)
            total_debit = round(sum(
                float(t['amount'].to_decimal()) if isinstance(t['amount'], Decimal128) else float(t['amount'])
                for t in transactions if t.get('transaction_type') == 'debit'
            ), 3)
...
@api_router.put("/daily-closings/{closing_id}", response_model=DailyClosing)
async def update_daily_closing(closing_id: str, update_data: dict, current_user: User = Depends(require_permission('finance.create'))):
    ...
    try:
        ...
        if 'actual_closing' in update_data:
            expected_closing = safe_float(existing_closing.get('expected_closing', 0.0))
            actual_closing = safe_float(update_data['actual_closing'])
        ...
        return DailyClosing(**decimal_to_float(updated_closing))
...
async def get_purchase_history_report(
    ...
    purchases = decimal_to_float(purchases)
    ...
    # Get StockMovements and Transactions
    stock_movements = await db.stock_movements.find(stock_query, {"_id": 0}).to_list(10000)
    stock_movements = decimal_to_float(stock_movements)
    transactions = await db.transactions.find(txn_query, {"_id": 0}).to_list(10000)
...
async def get_returns_summary_report(
    ...
    # Convert Decimal128 to float for calculations
    returns = [decimal_to_float(ret) for ret in returns]
    ...
    for ret in returns:
        ...
        # Get total_amount (already converted to float)
        total_amt = ret.get('total_amount', 0)
        ...
        # Money refunded (already converted to float)
        money_refunded += ret.get('refund_money_amount', 0)
        
        # Gold refunded (already converted to float)
        gold_refunded += ret.get('refund_gold_grams', 0)
...
async def get_transaction_delete_impact(transaction_id: str, current_user: User = Depends(require_permission('finance.view'))):
    ...
    return {
        ...
        "amount": round(safe_float(transaction.get("amount", 0)), 2),
...
async def delete_transaction(
    ...
    ACCOUNTING RULES:
    - For invoice payment transactions: Deletes both debit & credit entries, reverses invoice payment
    - For manual transactions: Deletes single transaction and reverses account balance
    ...
    # Handle invoice payment transactions (double-entry)
    if transaction.get("reference_type") == "invoice":
        invoice_id = transaction.get("reference_id")
        if not invoice_id:
            raise HTTPException(status_code=400, detail="Invoice reference missing for invoice payment transaction")
        
        # Fetch the invoice
        invoice = await db.invoices.find_one({"id": invoice_id, "is_deleted": False})
        if not invoice:
            raise HTTPException(status_code=404, detail="Related invoice not found")
        
        # Convert Decimal128 to float for calculations
        invoice = decimal_to_float(invoice)
        
        # Get payment amount from transaction
        payment_amount = float(transaction.get('amount', 0))
        
        # Find the paired transaction (debit/credit pair)
        # Both transactions have the same reference_id and amount
        paired_transactions = await db.transactions.find({
            "reference_type": "invoice",
            "reference_id": invoice_id,
            "amount": payment_amount,
            "is_deleted": False
        }).to_list(length=100)
        paired_transactions = decimal_to_float(paired_transactions)
        
        # Filter to find transactions created around the same time (within 5 seconds)
        current_created_at = transaction.get('created_at')
        paired_txns = [
            t for t in paired_transactions 
            if t['id'] != transaction_id 
            and abs((t.get('created_at') - current_created_at).total_seconds()) < 5
        ]
        
        # Validate invoice finalization status
        if invoice.get("status") == "finalized":
            raise HTTPException(
                status_code=400,
                detail="Cannot delete payment from finalized invoice. Payment is locked with the finalized invoice. To modify payments, you must first edit the invoice to draft status."
            )
        
        # Reverse the payment on the invoice
        current_paid_amount = float(invoice.get('paid_amount', 0))
        new_paid_amount = max(0, current_paid_amount - payment_amount)
        new_balance_due = float(invoice.get('grand_total', 0)) - new_paid_amount
        
        # Determine new payment status
        if new_paid_amount < 0.01:
            new_payment_status = "unpaid"
        elif new_balance_due < 0.01:
        ...
        # Clear paid_at if invoice becomes unpaid
        ...
        # Delete all paired transactions and reverse their account balances
        deleted_transaction_ids = [transaction_id]
        for paired_txn in paired_txns:
            deleted_transaction_ids.append(paired_txn['id'])
            
            # Reverse account balance for paired transaction
            paired_account_id = paired_txn.get('account_id')
            paired_amount = paired_txn.get('amount', 0)
            paired_type = paired_txn.get('transaction_type', 'debit')
            
            if paired_account_id:
                paired_account = await db.accounts.find_one({"id": paired_account_id, "is_deleted": False})
                if paired_account:
                    account_type = paired_account.get('account_type', 'asset')
                    reverse_type = 'credit' if paired_type == 'debit' else 'debit'
                    balance_delta = calculate_balance_delta(account_type, reverse_type, paired_amount)
                    
                    await db.accounts.update_one(
                        {"id": paired_account_id},
                        {"$inc": {"current_balance": balance_delta}}
                    )
            
            # Soft delete paired transaction
        ...
        # Reverse account balance for original transaction
        ...
        # Soft delete original transaction
        await db.transactions.update_one(
            {"id": transaction_id},
            {
                "$set": {
        ...
        await create_audit_log(
            ...
            {
                "payment_amount": payment_amount,
                "deleted_transactions": deleted_transaction_ids,
                "new_paid_amount": new_paid_amount,
                "new_balance_due": new_balance_due,
        ...
        return {
            "message": "Invoice payment deleted successfully. Payment reversed on invoice.",
            "transaction_id": transaction_id,
            "paired_transactions_deleted": len(paired_txns),
            "invoice_updated": True,
            "new_invoice_balance_due": new_balance_due
        }
    
    # Handle purchase payment transactions (single-entry)
    elif transaction.get("reference_type") == "purchase":
        purchase_id = transaction.get("reference_id")
        if not purchase_id:
            raise HTTPException(status_code=400, detail="Purchase reference missing for purchase payment transaction")
        
        # Fetch the purchase
        purchase = await db.purchases.find_one({"id": purchase_id, "is_deleted": False})
        if not purchase:
            raise HTTPException(status_code=404, detail="Related purchase not found")
        
        # Convert Decimal128 to float for calculations
        purchase = decimal_to_float(purchase)
        
        # Validate purchase is not locked
        if purchase.get("locked"):
            raise HTTPException(
                status_code=400,
                detail="Cannot delete payment from locked purchase. Purchase is finalized and fully paid."
            )
        
        # Get payment amount from transaction
        payment_amount = float(transaction.get('amount', 0))
        
        # Reverse the payment on the purchase
        current_paid_amount = float(purchase.get('paid_amount_money', 0))
        new_paid_amount = max(0, current_paid_amount - payment_amount)
        new_balance_due = float(purchase.get('total_amount_money', 0)) - new_paid_amount
        ...
        if account_id:
            ...
            if account:
                account_type = account.get('account_type', 'asset')
                transaction_type = transaction.get('transaction_type', 'credit')
                reverse_type = 'credit' if transaction_type == 'debit' else 'debit'
                balance_delta = calculate_balance_delta(account_type, reverse_type, payment_amount)
        ...
        return {
            "message": "Purchase payment deleted successfully. Payment reversed on purchase.",
            "transaction_id": transaction_id,
            "purchase_updated": True,
            "new_purchase_balance_due": new_balance_due
        }
    
    # Handle manual transactions (no reference)
    else:
...
async def create_return(
    ...
    try:
        ...
        # Convert Decimal128 to float for calculations
        return_data = decimal_to_float(return_data)
        ...
        return_obj = Return(
            ...
            account_id=account_id,
            account_name=None,  # Will be set at finalize time
        ...
        # Return draft return (NOT finalized)
        return {
            "message": "Return draft created successfully. Use finalize endpoint to complete the return.",
            "return": decimal_to_float(return_obj.model_dump()),
            "details": {
                "status": "draft",
                "finalized": False,
                "next_step": "Call POST /api/returns/{id}/finalize with refund details to complete"
            }
        }
...
async def get_returns(
    ...
    try:
        ...
        # Fetch returns
        cursor = db.returns.find(query, {"_id": 0}).sort("created_at", -1).skip(skip).limit(page_size)
...
async def get_return_by_id(
    ...
    return_doc = await db.returns.find_one({"id": return_id, "is_deleted": False}, {"_id": 0})
    if not return_doc:
...
async def update_return(
    ...
    try:
        ...
        # Create audit log (convert Decimal128 values for safe serialization)
        await create_audit_log(
            user_id=current_user.id,
            user_name=current_user.full_name,            
            module="returns",
            record_id=return_id,
            action="update",
            changes=decimal_to_float(update_fields)
        )
        
        # Fetch updated return
        updated_return = await db.returns.find_one({"id": return_id}, {"_id": 0})
    ...
    try:
        ...
        if return_type == 'sale_return':
            ...
            transaction = Transaction(
                ...
                transaction_number=transaction_number,
                date=datetime.now(timezone.utc),
                transaction_type="credit",  # Money going out to customer (credit decreases asset)
            ...
            await db.accounts.update_one(
                {"id": account_id},
                {"$inc": {"current_balance": _safe_decimal128(-round(refund_money_amount, 2), '0.001')}}
            )
            ...
            if sales_income_account:
                ...
                await db.transactions.insert_one(convert_transaction_to_decimal(income_transaction.model_dump()))
        ...
        if reference_type == 'invoice':
            ...
            if invoice:
                ...
                new_paid = safe_float(invoice.get('paid_amount', 0)) - refund_money_amount
                new_balance = safe_float(invoice.get('grand_total', 0)) - new_paid
                
                await db.invoices.update_one(
                    {"id": reference_id},
                    {
                        "$set": {
                            "paid_amount": _safe_decimal128(max(0, new_paid), '0.001'),
                            "balance_due": _safe_decimal128(max(0, new_balance), '0.001'),
        ...
        if party_id:
            ...
            if party and party.get('party_type') == 'customer':
                ...
                current_outstanding = safe_float(party.get('outstanding_balance', 0))
                ...
                await db.parties.update_one(
                    {"id": party_id},
                    {"$set": {"outstanding_balance": _safe_decimal128(round(new_outstanding, 2), '0.001')}}
                )
        ...
        # ========================================================================
            ...
            # CRITICAL BUSINESS RULE: DO NOT AUTO-UPDATE INVENTORY
            # Inventory adjustments must be done manually after physical verification
            ...
            for item in return_doc.get('items', []):
                ...
                if weight_grams > 0:
                    ...
                    pending_adjustments.append({
                        "item_description": item.get('description'),
                        "qty": -qty,  # Negative because stock goes out
                        "weight_grams": -round(weight_grams, 3),  # Negative because stock goes out
                        "purity": item.get('purity'),
                        "action_required": "manual_stock_adjustment",
                        "notes": f"Purchase Return - {return_doc.get('return_number')} - Requires manual inventory adjustment after physical verification"
                    })
            ...
            if reference_type == 'purchase':
                ...
                if purchase:
                    ...
                    new_balance = safe_float(purchase.get('balance_due_money', 0)) - refund_money_amount
                    
                    await db.purchases.update_one(
                        {"id": reference_id},
                        {"$set": {"balance_due_money": _safe_decimal128(max(0, new_balance), '0.001')}}
                    )
        ...
        # Build response message - Both sales and purchase returns require manual inventory action
            ...
            if transaction_id:
                ...
                if transaction:
                    ...
                    if account_id:
                        ...
                        await db.accounts.update_one(
                            {"id": account_id},
                            {"$inc": {"current_balance": _safe_decimal128(balance_change, '0.001')}}
                        )
                    # Delete transaction
...
async def list_backups(request: Request, current_user: User = Depends(require_permission('audit.view'))):
    """
...
async def create_backup(request: Request, current_user: User = Depends(require_permission('audit.view'))):
    """
    Manually trigger a database backup
    Requires 'audit.view' permission (admin only)
    Rate limited to 5 backups per hour
    """
    try:
        ...
        await create_audit_log(
            ...
            "create_manual",
            {"success": result['success'], "size_mb": result.get('size_mb')}
        )
...
async def restore_backup(
    ...
    Requires 'audit.view' permission (admin only)
    Rate limited to 2 restores per hour
    
    WARNING: This will overwrite current database data!
    Set drop_existing=True to drop existing collections before restore
    """
    try:
        # Additional safety check
        if not backup_filename or not backup_filename.endswith('.tar.gz'):
            raise HTTPException(
                status_code=400,
                detail="Invalid backup filename"
            )
        
        result = backup_manager.restore_backup(backup_filename, drop_existing)
...
@api_router.post("/backups/verify/{backup_filename}")
@limiter.limit("10/minute")
...
# Build regex pattern for wildcard origins (e.g., https://*.vercel.app)
# This allows secure subdomain matching instead of allowing ALL origins
...
for origin in cors_origins_list:
    if '*' in origin:
        ...
        # https://*.vercel.app becomes https://.*\.vercel\.app
...
cors_origin_regex = '|'.join(regex_patterns) if regex_patterns else None

# ============================================================================
# AUTOMATIC DECIMAL128 CONVERSION MIDDLEWARE
# ============================================================================
class Decimal128AutoConversionMiddleware(BaseHTTPMiddleware):
    """
    Middleware that automatically converts all Decimal128 values in responses to float.
    
    This ensures:
    1. No manual conversion needed in endpoint handlers
    2. All numeric calculations work correctly (Decimal128 → float)
    3. JSON serialization never fails due to Decimal128
    4. Consistent data types across all API responses
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if response.headers.get("content-type", "").startswith("application/json"):
            ...
            try:
                # Parse JSON and apply auto-conversion
                data = json.loads(body)
                converted_data = auto_convert_response(data)
...
# Add Decimal128 auto-conversion middleware FIRST (before CORS)
app.add_middleware(Decimal128AutoConversionMiddleware)

# Add CORS middleware with both exact origins and regex pattern support
app.add_middleware(
    CORSMiddleware,
